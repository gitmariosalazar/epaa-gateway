...
export class ObservationsGatewayController implements OnModuleInit {
  private readonly logger: Logger = new Logger(ObservationsGatewayController.name);
  constructor(
  ...
  async getObservationDetailsByCadastralKey(@Param('cadastralKey') cadastralKey: string, @Req() request: Request): Promise<ApiResponse> {
    try {
      ...
      return new ApiResponse(
        `Observation details for cadastral key ${cadastralKey} retrieved successfully!`, response, request.url
      );
    } catch (error) {
