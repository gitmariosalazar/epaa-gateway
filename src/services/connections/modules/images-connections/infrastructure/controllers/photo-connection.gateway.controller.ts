import { Body, Controller, Get, Inject, Logger, OnModuleInit, Param, Post, Req, UploadedFiles, UseInterceptors } from "@nestjs/common";
...
export class PhotoConnectionGatewayController implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    this.photoConnectionClient.subscribeToResponseOf('photo-connection.create-photo-connection');
    this.photoConnectionClient.subscribeToResponseOf('photo-connection.get-photo-connections-by-cadastral-key');
    this.logger.log('Response patterns:', this.photoConnectionClient['responsePatterns']);
  ...
  async createPhotoConnections(
    ...
    try {
      ...
      const photoConnectionDtos: CreatePhotoConnectionRequest[] = images.map((image) => {
        ...
        const finalPath = join('/home/sigepaa/sigepaa/images/connections', finalFilename);
  ...
  async getPhotoConnectionsByCadastralKey(
    @Req() request: Request, @Param('cadastralKey') cadastralKey: string
  ): Promise<ApiResponse> {
    try {
      ...
      this.logger.error('Error retrieving photo connections by cadastral key:', error);
      throw new RpcException(error);
    }
  }
}