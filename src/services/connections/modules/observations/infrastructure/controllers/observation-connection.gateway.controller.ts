...
export class ObservationConnectionGatewayController implements OnModuleInit {
  private readonly logger = new Logger(ObservationConnectionGatewayController.name);
  ...
  async onModuleInit() {
    this.kafkaClient.subscribeToResponseOf('observation-connection.create-observation-connection');
    this.kafkaClient.subscribeToResponseOf('observation-connection.get-observation-connections-by-observation-id');
    this.kafkaClient.subscribeToResponseOf('observation-connection.get-observation-connections-by-connection-id');
    this.kafkaClient.subscribeToResponseOf('observation-connection.get-all-observation-connections');
    await this.kafkaClient.connect();
  }

  @Post('create-observation-connection')
  @ApiOperation({ summary: 'Create a new observation connection' })
  async createObservationConnection(@Req() request: Request,
    ...
    try {
      ...
      return new ApiResponse('Observation connection created successfully', response, request.url);
    } catch (error) {
  ...
  async getAllObservationConnections(@Req() request: Request): Promise<ApiResponse> {
    try {
      const response = await sendKafkaRequest(
        this.kafkaClient.send('observation-connection.get-all-observation-connections', {})
      );
      return new ApiResponse('All observation connections retrieved successfully', response, request.url);
    } catch (error) {
  ...
  async getObservationConnectionsByObservationId(
    ...
    try {
      ...
      return new ApiResponse('Observation connections retrieved successfully', response, request.url);
    } catch (error) {
      this.logger.error('Failed to retrieve observation connections by observation ID', error);
      throw new RpcException(error);
    }
  }
  ...
  async getObservationConnectionsByConnectionId(
    ...
    try {
      ...
      this.logger.error('Failed to retrieve observation connections by connection ID', error);
      throw new RpcException(error);
    }
  }
